...
pygame.display.set_caption("JOHOR CHAMPIONSHIP")
...
RED_SPACESHIP_IMAGE = pygame.image.load(
    os.path.join("Assets", "yassin.png"))
...
def draw_window(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    ...
    red_health_text = HEALTH_FONT.render(
        "(KEYS Arrow_Keys LShift) Health: " + str(red_health), 1, BLUE)
    yellow_health_text = HEALTH_FONT.render(
        "(KEYS WASZ LShift) Health: " + str(yellow_health), 1, WHITE)
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL > 0:  # LEFT
        yellow.x -= VEL
    if keys_pressed[pygame.K_s] and yellow.x + VEL + yellow.width < BORDER.x:  # RIGHT
        yellow.x += VEL
    if keys_pressed[pygame.K_w] and yellow.y - VEL > 0:  # UP
        yellow.y -= VEL
    if (
        keys_pressed[pygame.K_z] and yellow.y +
            VEL + yellow.height < HEIGHT - 15
    ):  # DOWN
        yellow.y += VEL
...
def main():
    ...
    while run:
        ...
        winner_text = ""
        if red_health <= 0:
            winner_text = "UMNO WINS"

        if yellow_health <= 0:
            winner_text = " BERSATU WINS!"
